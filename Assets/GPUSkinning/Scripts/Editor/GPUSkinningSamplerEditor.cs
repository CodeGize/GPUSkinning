...
public class GPUSkinningSamplerEditor : Editor
{
    ...
    private void PreviewDrawGrid()
    {
        if (gridGos == null)
        {
            gridMtrl = new Material(Shader.Find(GridShader));
    ...
    private const string ArrowPath = "Assets/GPUSkinning/EditorRes/Model/GPUSkinningSamplerEditor_Arrow.fbx";
    private const string UnlitShader = "GPUSkinning/GPUSkinningSamplerEditor_UnlitColor";
    private const string GridShader = "GPUSkinning/GPUSkinningSamplerEditor_Grid";
    ...
    private void PreviewDrawArrows()
    {
        if (arrowGos == null)
        {
            ...
            {
                GameObject go = Instantiate(AssetDatabase.LoadAssetAtPath<GameObject>(ArrowPath));
                go.hideFlags = HideFlags.HideAndDontSave;
                arrowGos[i] = go;

                arrowMtrls[i] = new Material(Shader.Find(UnlitShader));
    ...
    private void PreviewDrawBounds()
    {
        if (boundsGos == null)
        {
            boundsMtrl = new Material(Shader.Find(UnlitShader));
